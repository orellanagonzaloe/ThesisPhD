...
% TMPChange\commentyellow{Ver dudas sobre pi0}
. Para reducir la identificación errónea se aplican entonces una serie de criterios de identificación y aislamiento, basados en las formas de los depósitos de energía, que permiten discriminar estos procesos de los procesos \textit{prompt} 
% TMPChange \commentyellow{Ver dudas sobre la definicion de prompt}
. 
...
% TMPChange\commentred{Poner variablezs de electrones. Entender y traducir bien algunas variables. Considerar: For variables calculated in the first EM layer, if the cluster has more than one cell in the phi direction at a given eta, the two cells closest in phi to the cluster barycentre are merged and the definitions below are given in terms of this merged cell.}
  ...
  $1^{\text{er}}$ capa del ECAL & $E_{\text{ratio}}$ & Ratio of the energy difference between the maximum energy depositand the energy deposit in a secondary maximum in the cluster to thesum of these energies & \xmark & \cmark & \cmark \\
   ...
   & $f_{\text{side}}$ & Energy fraction outside core of three central cells, within seven cells & \xmark & \xmark & \cmark \\
...
% TMPChange\commentyellow{los nombres deberían llevar el FixedCut o FC}
}
...
% TMPChange\commentyellow{Me gustaría poner algo de cuánto interactúan los muones con el detector, y algo de los muones cósmicos}
...
\begin{itemize}
  ...
  % TMPChange\commentyellow{Ver dudas sobre muones en el MS}
...
% TMPChange\commentyellow{kinky: retorcida? compleja? traducir mejor!} 
...
% TMPChange\commentyellow{agregar alguna referencia a análisis/definición de estas partículas}
...
% TMPChange\commentyellow{mencionar al menos los WPs que se usan en el análisis}
...
% TMPChange\commentyellow{Posibilidad de agregar: hadronic showers inherently rely on interactions with the strong force to start electromagnetic cascades, typically via the creation of pi0 and pipm . However, these strong interactions result in a substantial amount of energy going into splitting nuclei, and thus overcoming the binding energy. This energy is not observed}
...
% TMPChange\commentyellow{HW: en general es un solo vértice secundario. Te referís acá al algo tipo B- > JPsi ->Ks ?
...
Como se mencionó en % TMPChange\tosolve{definición MET}
