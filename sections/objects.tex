...
características de cada una de ellas, haciendo que cada una de las partículas anteriores depositen señales distintivas, permitiendo su reconstrucción e identificación. La Figura \ref{particulasATLAS} muestra un esquema de las distintas señales producidas por cada una de las partículas en el detector ATLAS. Todos los procesos de reconstrucción descriptos se realizan una vez que el evento cumplió los requisitos del \trigger y fue almacenado
(\textit{offline}).

\begin{figure}
\centering
\caption{Esquema de los distintos tipos de señal que pueden dejar las partículas en el detector ATLAS \commentyellow{Puede omitirse esta imagen} \commentNotaIII }
...
\begin{figure}
\centering
\caption{Esquema del cono utilizado para el cálculo de la variable de aislamiento calorimétrico \commentNotaIII }
...
\begin{table} 
\centering
\caption{Definición de los WPs de aislamiento para fotones y electrones.\commentyellow{los nombres deberían llevar el FixedCut o FC}}
...
\begin{itemize}
	...
	\item Muones Calorimétricos (CT): trazas del ID que están asociadas a depósitos de energía en el calorímetro compatibles con una partícula mínimamente ionizante. Este tipo de muones son los de menor pureza pero permite detectarlos en regiones donde el MS está parcialmente instrumentado. \commentHW{Y también los de baja energía que no llegan al MS ?} \commentgreen{Hay muones que no lleguen al MS?}
...
\commentgreen{hadronic showers inherently rely on interactions with the strong force to start electromagnetic cascades, typically via the creation of pi0 and pipm . However, these strong interactions result in a substantial amount of energy going into splitting nuclei, and thus overcoming the binding energy. This energy is not observed}
\commentHW{No pondira esto solo, las razones de la necesidad de calbirar son muchas. Esta entre otras.}
\commentgreen{Me pareció que lo que ponía este parrafo estaba bueno. Capaz lo podríamos poner aclarando que ademas hay otras razones ?s}
...
\commentHW{en general es un solo vértice secundario. Te referís acá al algo tipo B- > JPsi ->Ks ?}
\commentgreen{No recuerdo bien de donde lo saqué, pero entiendo al producirse un b genera una cadena de decaimientos a quarks de menor masa, y en cada etapa se produce un vértice. En esa cadena que ponés habría 2 vértices secundarios, el primario es el que produjo al B, o no?}
