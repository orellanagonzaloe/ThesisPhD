...
Los aceleradores pueden ser caracterizados no solo por su energía de centro de mas, sino también por su \textbf{Luminosidad Instantánea} ($\mathcal{L}$), que mide el número de colisiones por unidad de área que ocurren en un período de tiempo, y se define como: 
...
El detector interno es el más próximo al haz y su función principal es la reconstrucción de las trazas de las partículas cargadas, que a su vez sirve para medir la dirección, momento y carga de la misma, junto con la reconstrucción de los vértices primarios. Para ello combina detectores de muy alta resolución cerca del haz, junto con detectores continuos de trazas en la zona más alejada. 
...
El detector interno está contenido dentro del solenoide superconductor, y mide \magn{6.2}{m} de largo y \magn{2.1}{m} de diámetro. El ID se compone de tres submódulos que se describen a continuación, y las Figuras \ref{fig:pixel_1} y \ref{fig:pixel_23} muestran un esquema del mismo.
