...
% TMPChange \commentyellow{Este capítulo en particular tiene muchas siglas, italicas y nombre en inglés que no se si se deben traducir. Puede que haya que encontrarle la vuelta para que quede prolijo}
...
% TMPChange \commentyellow{encontrar alguna utilidad más capaz?}
...
% TMPChange \commentyellow{justificar/motivación de por qué explicamos el método del Zrad si los triggers que usamos en el análisis son medidos con BS?}
...
% TMPChange \commentyellow{agregar el plot que describe la cadena de algoritmos} 
...
% TMPChange \commentyellow{agregar una imagen de esto, no es fácil explicarlo solo con texto}. 
...
% TMPChange \commentred{entender y traducir} 
...
% TMPChange \commentred{esto no lo entendí, lo traduje directo} 
...
% TMPChange \commentyellow{no me queda claro donde entra el conoa acá...} 
...
% TMPChange \commentyellow{Me resulta un poco confuso hacer esta sección sin haber hecho la sección de detector y trigger, por lo que pueden aparecer cosas que capaz haya que mover al principio} \commentyellow{Puede que me haya quedado información repetida con lo que escribí anteriormente}
...
% TMPChange \commentyellow{Pensar una forma de expresarlo mejor}
...
% TMPChange \commentyellow{Puede que no esté siendo muy específico, las dereivations EGAM tienen muchos OR}. 
...
\centering
  ...
  \includegraphics[width=0.5\textwidth]{images/h_mllg_mll.pdf}
	...
	% TMPChange \commentyellow{Voy a describirlo un poco más} 
...
% TMPChange \commentHW{Falta dar algún valor de los resultados de las incertezas sistemáticas y justificar algo de porque se tomaron esos criterios.}

En la Figura \ref{trigEff} se pueden observar los resultados de las eficiencias en función de las distintas variables. 
% TMPChange \commentyellow{Agregar la comparación con BS (que da mas eficiente), puedo aprovechar para mencionar BS y citar alguna tesis que describa este método (joaco).}
...
% TMPChange \commentyellow{Voy a agregar lo mismo pero para el resto de los años} \commentNotaIII
}
...
% TMPChange \commentHW{Falta comentar algo en general sobre los resultados que encontras, por el lado de ...las eficiencias del detector son altas y los SF cercanos a uno lo cual deja en evidencia el buen diseño y construcción del sistema de trigger y las simulaciones...}
...
% TMPChange \commentyellow{Había pensado que capaz puedo agregar a la derecha de este plot las eff vs pt y plotear juntos datos y MC, y abajo el ratio que seria el SF.} 
% TMPChange \commentNotaIII
}
