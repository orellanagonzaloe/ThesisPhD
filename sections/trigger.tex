...
\chaptermark{Eficiencia del trigger de fotones}
 ...
 % \commentyellow{Este capítulo en particular tiene muchas siglas, italicas y nombre en inglés que no se si se deben traducir. Puede que haya que encontrarle la vuelta para que quede prolijo} Criterio para italicas ya definido. Las siglas y los terminos en ingles no se todavia...
...
\tosolve{Justificar o motivar de una mejor forma el por qué explicamos el método del Zrad si los triggers que usamos en el análisis son medidos con Bootstrap}
...
\tosolve{agregar el plot que describe la cadena de algoritmos} 
...
utiliza la información del calorímetro en la región central ($|\eta|<2.5$) para construir las RoIs, que consisten en torres (\textit{trigger towers}) de $4\times4$ celdas del calorímetro de $0.1\times0.1$ en $\eta$ y $\phi$. Un algoritmo (\textit{sliding-window} \cite{Lampl:1099735}) busca los conjuntos de celdas de $2\times2$ cuya suma de energía transversa de uno de los cuatro posibles pares de celdas vecinas más cercanas ($1\times2$ o $2\times1$) supere el umbral de energía que define al trigger. 
 \tosolve{Una imagen de esto facilitaría mucho la explicación}. 
...
dentro de una ventana de $3\times7$ ($\Delta\eta\times\Delta\phi = 0.075\times0.175$) centrada en la celda semilla. Para calcular la energía acumulada 
 \tosolve{Esto no lo entendí, lo traduje directo} 
...
% / Eso lo veré una vez este terminado todo... / 
...
\resizebox{\linewidth}{!}{
	\begin{tabular}{ l | c | c | c }
		...
		Primario de un fotón & \texttt{HLT\_g120\_loose} & \multicolumn{2}{c}{\texttt{HLT\_g140\_loose}} \\

		\hline
		
		Primario de dos fotones & \multicolumn{2}{c|}{\texttt{HLT\_g35\_loose\_g25\_loose}} & \texttt{HLT\_g35\_medium\_g25\_medium} \\
...
La medida de la eficiencia de cada trigger utiliza los datos tomados en el año correspondiente al mismo, listados en la Tabla \ref{TrigMenu}. En el caso de que el trigger o una de sus componentes se haya configurado con un prescale, el mismo se emplea en modo rerun para la medida de la eficiencia. 
...
La incertidumbre estadística para las eficiencias se obtiene como el intervalo de confianza de un estimador de Bayes con el método de Jeffrey \cite{jeffrey} \tosolve{Estudiar esto}. Las incertezas sistemáticas se obtienen a partir de las variaciones en las eficiencias al modificar algunas de las selecciones empleadas en el método, principalmente las asociadas a los leptones para evitar un posible sesgo en esa selección. El requisito sobre las masas invariantes se varió de $36<m_{ll}<87$ GeV a $44<m_{ll}<79$ GeV, y de 
$82<m_{ll\gamma}<100$ GeV a $88<m_{ll\gamma}<94$ GeV. Se modificó el requerimiento de identificación de los leptones a \texttt{Tight} y \texttt{Medium}, y de aislamiento a \texttt{FixedCutTight}. En general las incertidumbres tomaban valores entre \tosolve{Buscar valores!}.
...
\tosolve{Tanto para eficiencias como para SF, debería mencionar la tabla con los triggers para los cuales se calcularon?}
...
\begin{figure}
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/1.png} % egamma reco
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/2.png} % egamma reco
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/3.png} % egamma reco
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/4.png} % trigger paper
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/5.png} % 1902.04655
\end{figure}

\begin{figure}
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/6.png} % 1611.09661
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/7.png} % 1611.09661
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/8.png} % 1810.05087
  \includegraphics[width=0.5\textwidth]{images_tmp/trigger/9.png} % 1810.05087
\end{figure}