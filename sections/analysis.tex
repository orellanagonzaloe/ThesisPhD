...
% produciendo jets como parte de su decaimiento a neutralino sin carga de color
a su paso y generando el estado final buscado. Una cadena de decaimiento típica de este modelo se puede observar en la Figura \ref{fig:phb_feyn}.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{images/analysis/gogo-qqqqbbphGG-h.pdf}
  \caption{Ejemplo de cadena de decaimiento del modelo supersimétrico en el que se centra esta Tesis. \label{sacar los b del higgs, poner cadena larga} \tosolve{Produccion fuerte de gluinos y su decaimiento en el estado final estudiado}}
  \label{fig:phb_feyn}
\end{figure}
...
% \tosolve{Acá había encontrado una fórmula mediante un fit para M(mu), pero no se si es importante} Mejor no ponerlo...
...
% Como el modelo es un GGM, se fijó la masa del \gravino en $1$ eV \tosolve{esto no es tan así, la masa variaba de acuerdo al mu, lo deberia poner?}. Corregido
La masa del \gravino, la LSP impuesta por los modelos GGM, se determinó del orden de \magn{1}{eV}.
...
Como ejemplo se muestra en la Figura \ref{fig:mass_spec} un espectro de masas para uno de los puntos de señal con $(M_3, \mu) = (1800\,\gev, -1050\,\gev)$. Un ejemplo de posibles decaimientos de los gluinos para ese mismo punto de señal se puede observar en la Figura \ref{fig:gluino_decays}. 
...
\begin{figure}
  ...
  \caption{Espectro de masas de las partículas supersimétricas para el punto de señal con $(M_3, \mu) = (1800\,\gev, -1050\,\gev)$. \tosolve{hay una versión mejor de esta imagen}}
  \label{fig:mass_spec}
\end{figure}


% \tosolve{Hay mas graficos de BR para poner, hacen falta? no lo creo...} No hace falta
...
Para modelar los fondos con fotones `reales' se utilizaron simulaciones de MC, mientras que para aquellos con fotones `falsos' se utilizaron técnicas basadas en datos. 
% \tosolve{mencionar que para la optimizacion sí se usaron MC para los fakes?} Na...
...
Los fondos del SM con fotones `reales' fueron modelados utilizando simulaciones de MC, los cuales consisten en: \phj, \wph, \ttbarph, \wphph, \zph, \zphph, \phph. Los tres primeros son considerados los de mayor impacto en el análisis y por ende fueron normalizados en respectivas regiones de control. Para el resto de los fondos solo se utilizaron las simulaciones con las normalizaciones usuales (Sección \ref{sec:mc_weights}). 
% reescrito
...
Cabe mencionar que también el fondo \znunuph tiene un impacto considerable en algunas regiones del análisis, pero la dificultad para diseñar una región de control dedicada llevó a la decisión de utilizarlo con las normalizaciones usuales. 
% \tosolve{sigue estando mal poner normalizaciones usuales si ya lo describi en una seccion ?}
...
% \tosolve{algún dato más debería poner, cual?}. % nope
...
% \tosolve{Esto cuando se utilizaría? Me parece que mirar las formas quedó de un método viejo no? Ahora solo miramos número de eventos. O es para la correlación?} % lo cambie a variables y listo
En el contexto de este método se definen fotones aislados como aquellos que pasan el WP \texttt{FixedCutTight},
 % \tosolve{esta bien esto?} Si...
 $-20\,\gev<\ETiso<0\,\gev$ y $0<\pTiso<0.05$, y los no-aislados aquellos con $8\,\gev<\ETiso<80\,\gev$ o $0.15<\pTiso<1$.
% \tosolve{capaz deba definir estas variables aca, o en la parte de fotones} Done, en fotones
...
El método \texttt{ABCD}, utilizado en este análisis en particular, preselecciona eventos con al menos un fotón con $\pt>1450\,\gev$, al menos dos jets y ningún leptón, cuyos requisitos son idénticos a los que se usan en el análisis descriptos en la Sección \ref{sec:selection}. A partir de ello define cuatro regiones \cite{ATL-COM-PHYS-2016-1626}:
...
Distintas correcciones se realizan sobre este método. El primero es considerar la posibilidad de una contribución de fotones reales de señal en las regiones B, C y D. La misma es estimada utilizando simulaciones de MC con fotones `reales' a nivel generador, y es luego restada 
% \tosolve{revisar tiempo del verbo}
a los datos observados para asegurar de este modo solo el conteo de eventos de fondo en cada región. Adicionalmente se considera una posible correlación entre variables calorimétricas y de identificación, que se puede agregar como un factor multiplicativo a la Ecuación \ref{eq:jfakes_ff}. Ese factor se define como:
...
% \begin{table}[!ht]
%   \centering
%   \caption{Valores obtenidos para los factores de reconstrucción errónea de jets en fotones en función del \pt y $|\eta|$ del fotón, y la \met del evento. Las incertidumbres son tanto estadísticas como sistemáticas. \tosolve{Redondear}}
...
% \tosolve{tengo que pensar mejor estas fórmulas, pero va por aca la cosa} O sea están bien, se omitio un paso nomas que se basa en el footnote...
...
% \tosolve{no me convence poner (\absEta) en los siguientes terminos}
...
% \tosolve{no sabia esto!!!}. Era importante cuando no teniamos eventos en algunas regiones en 2016, ahora no pasa eso

% \tosolve{esto tambien se hace para jfakes, por que nunca lo hacemos explicito?} Agregar
...
% \tosolve{hernan sugiere sacarlo, pero despues justificarlos, no se si habria que sacarlo} 
que se describe a continuación. La misma se emplea para aplicar un requisito de solapamiento (\textit{overlap removal}) entre los distintos objetos del evento y posteriormente empleados para el cálculo de \met \tosolve{Ojo con esto al final no se usaba para met, no?}. Finalmente a los fotones, jets y leptones se les aplica una selección denominada \textit{signal}, siendo estos objetos los que definen las distintas regiones del análisis. Adicionalmente a los fondos de MC se les aplica una selección a nivel generador para evitar un solapamiento o doble conteo entre muestras. Por ejemplo, a las muestras de \zph, \wph, \ttbarph y \phj se les solicita que tengan uno y solo un fotón a nivel generador, y en cambio a las \zphph y \wphph dos fotones. 

% \tosolve{mencionar a VgammaORTool} Done

% \tosolve{como son cortos, capaz los siguientes items podrían ser subsubsection} Yep
...
% \tosolve{chequear}. Yep, pero solo para MC, ni aclarar igual


\section{Definición de las regiones del análisis}

Una vez obtenidas las muestras de señal y una primera estimación de los fondos del SM, el objetivo del análisis es poder diseñar regiones de señal que permitan hacer una discriminación entre ambos,
% Este proceso se denomina optimización y fue 
como se describe en la Sección \ref{sec:exp_sig}. Para ello se obtiene del espacio de parámetros, la combinación de requisitos o cortes que maximice la significancia esperada.
 % (Ecuación \ref{Agregar}). 
...
% Para seleccionar eventos con al menos un fotón es necesario
...
% \tosolve{mencionar lo que paso con el analisis phb? que se pensaba que los bjets del higgs podian llegar a servir para una nueva SR pero la imposibildiad de reconstruirlos correctamente no presentaba ninguna ventaja con estas SR, que en definitiva ya eran sensibles al analisis} Nope
...
Las incertezas experimentales están asociadas a la estimación de los fondos, ya sea por incertidumbres provenientes de la simulación del detector y la reconstrucción de objetos, como en los métodos de estimación de fondos basados en datos.
...
% \tosolve{no se si esta definido} Si
...
% \tosolve{por que solo para la carga de los muones? supongo por como se mide la carga, algo del charge flip tambien, podria averiguar}

\subsubsection{Incertezas asociadas a jets}

Las incertezas asociadas a jets se estiman siguiendo la metodología descripta en las Referencias \cite{PERF-2011-03} y \cite{ATLAS-CONF-2015-037}, las cuales aportan una gran cantidad de parámetros nuisance. entre ellas están la incertidumbre asociadas a la resolución y escala de energía, las eficiencias de identificación de sabor del jet y la incertidumbre en el JVT. 

\tosolve{Me pareció que describirlas en detalle era demasiada información sin precisiones...}.
\tosolve{Bueno, deberia detallar un poco mas, tal vez inclusive en la de fotones y leptones}.
...
\begin{table}[ht!]
  \centering
  \caption{Valores porcentuales promedios de las incertidumbres teóricas de cada muestra de fondo en cada región. Los fondos con baja estadística no afectan los resultados en la región y sus incertidumbres se fijan conservadoramente a 100\%.}
  \resizebox{.9\textwidth}{!}{
...
% \tosolve{poner algun plot de las barras de sistematicos?} nope
