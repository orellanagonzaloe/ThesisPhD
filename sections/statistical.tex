...
Una vez que la estimación de los fondos son corroboradas mediante su respectiva validación en las distintas VRs, se procede a observar los datos en las SRs (\textit{unblinding}). En la Figura \ref{fig:region_scheme} se muestra un esquema del diseño de las regiones del análisis, donde se representa la normalización de los fondos en una región de control la cual extrapola dicha normalización a las demás regiones, junto con un conjunto de regiones de validación intermedias entre las regiones de control y señal. En la misma se puede observar como las regiones de control y validación son ortogonales a las regiones de señal.

\begin{figure}
	\centering
  \includegraphics[width=0.42\textwidth]{images/statistics/regions.pdf}
  \caption{Esquema del diseño de las regiones del análisis con una región de control la cual extrapola la normalización a las demás regiones, junto con regiones de validación intermedias a estas y a las regiones de señal. Tanto las regiones de control como validación son ortogonales a las regiones de señal.}
  \label{fig:region_scheme}
\end{figure}
...
\section{Criterios para el descubrimiento de nueva física}
...
\begin{equation}
	p_{\mu} = \int_{q_{\mu, \text{obs}}}^{\infty} f(q_\mu|\mu)dq_\mu \equiv \text{CL}_{s+b} \quad\quad\quad \text{y} \quad\quad\quad 1-p_b = \int_{q_{\mu, \text{obs}}}^{\infty} f(q_\mu|0)dq_\mu \equiv \text{CL}_{b}
	\label{ec:pvalue_mu}
\end{equation}
...
A partir del likelihood obtenido se calcula el estadístico de prueba para descubrimiento observado ($q_{0, \text{obs}}$), y con ello el p-value del mismo. Dicho valor permite  afirmar si la observación corresponde al descubrimiento de un nuevo fenómeno.
