...
(aunque no necesariamente tengan masa) % este comentario capaz se pueda omitir
...
El SM se formula como una teoría cuántica de campos, en general considerada efectiva ya que por ejemplo describe los fenómenos en una escala baja de energía en la que la gravedad es despreciable. A su vez se compone de teorías de \textit{gauge} en las que a partir de imponer simetrías en el lagrangiano, no solo están asociados cantidades conservadas como bien enuncia el Teorema de Emily Noether \cite{noether}, sino que también implica la existencia de interacciones mediadas por bosones de gauge. A continuación se realiza una breve descripción matemática del SM basada principalmente en la Referencia \cite{gkane}.
...
% \tosolve{Tal vez no sea correcto decir 'producto de la combinación de esos estados'}
% Gordon, pag 49.  
...
% \tosolve{el 'se interpreta' viene porque a piori no sabemos su carga, el 0 es notacion nomas}
...
% \tosolve{esto es tecnicamente el mecanismo de higgs entonces?}
...
% \tosolve{Creo que quedaría mejor m minúscula} Sip
...
\noindent
siendo este ahora sí invariante de $SU(2)$. 
% \tosolve{que otra invarianza? la completa del SM?}
...
% \tosolve{Hasta el final, esta igual a la tesis de Tincho}
...
Luego de la interacción a alta energía, cada partón del estado final comienza a radiar gluones,
...
% tosolve{cito a esa ecuacion porque es la introduccion del martin}
% \tosolve{Creo que quedaría mejor h minúscula} Se... convencion higgs siempre minuscula?
...
% \tosolve{mencionar alternativas para solucionar esto?} Nop
% \tosolve{
...
Por otro lado, los bosones de gauge del SM deben estar contenidos en un supermultiplete vectorial con sus respectivos supercompañeros denominados \textit{gauginos}. El gluón tiene un supercompañero de spin $1/2$ denominado \textit{gluino} ($\gluino$). Por su parte, la simetría $SU(2)_L\times U(1)_Y$ asociada a los bosones de gauge $W^+$, $W^0$, $W^-$ y $B^0$ tienen sus supercompañeros $\widetilde{W}^+$, $\widetilde{W}^0$, $\widetilde{W}^-$ y $\tilde{B}^0$, llamados \textit{winos} y \textit{bino}. Luego de la ruptura de simetría electrodébil los estados de gauge $W^0$ y $B^0$ se mezclan en los estados de masa $Z^0$ y $\gamma$, y de la misma forma lo hacen los $\widetilde{W}^0$ y $\widetilde{B}^0$ para dar lugar al \textit{zino} ($\widetilde{Z}^0$) y \textit{photino} ($\tilde{\gamma}$).
...
\begin{table} 
	...
	\tosolve{Entender esto último y por qué la negrita y la barra en los números}}
...
% tosolve{Using Λ UV ∼ M P and λ ∼ 1 in eq Delta h}
...
\noindent
donde $M_1$, $M_2$ y $M_3$ son los términos de masa del bino, wino y gluino. Las matrices complejas de $3\times3$ con unidades de masa $\textbf{a}_\textbf{u}$, $\textbf{a}_\textbf{d}$ y $\textbf{a}_\textbf{e}$, se corresponden con los acoplamientos de Yukawa del superpotencial. 
...
% \tosolve{en la mitad es igual a la tesis de joaco}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{images/theory/sm_susy.pdf}
...
\tosolve{se puede mencionar algo sobre los $t_{1,2}$ y $b_{1,2}$}
...
% \tosolve{la masa de h0 y H0 son iguales?}
...
% \tosolve{Esto lo puse también en el capitulo de EWK, capaz mejor queda aca} Oka
